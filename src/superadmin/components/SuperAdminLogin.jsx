...
const SuperAdminLogin = () => {
    ...
    const handleLogin = async (e) => {
        ...
        try {
            if (loginMethod === 'otp' && mode === 'login') {
                // Request OTP
                const data = await authService.otpRequest(loginForm.username);
                if (data.success) {
                ...
                // Verify OTP
                const data = await otpService.verifyOtp(tempUser.mobile, enteredOtp);
                if (data.success) {
                ...
                if (result.success) {
                    ...
                    setLoginError(result.message || 'Invalid credentials or system access denied.');
                }
            }
        } catch (err) {
    ...
    const handleForgot = async (e) => {
        ...
        setIsLoading(true);
        try {
            const data = await authService.forgotPassword(forgotForm);
            if (data.success) {
                ...
                <form onSubmit={handleRegister} className="space-y-3.5">
                    <div className="grid grid-cols-2 gap-2">
                        <div className="relative">
                            ...
                            <input type="text" placeholder={t('name_label')} value={registerForm.name}
                                ...
                                className="w-full pl-8 pr-3 py-2 bg-white border border-slate-300 rounded-lg focus:border-indigo-500 outline-none text-[12px] font-medium text-slate-900" />
                        </div>
                        <div className="relative">
                    ...
                    <div className="relative">
                        ...
                        <textarea placeholder="Full Address" value={registerForm.address}
                            ...
                            className="w-full pl-8 pr-3 py-1.5 bg-white border border-slate-300 rounded-lg focus:border-indigo-500 outline-none text-[12px] font-medium text-slate-900 h-[40px] resize-none"></textarea>
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-2">
                        <div className="relative">
                            ...
                            <select value={registerForm.role} onChange={e => setRegisterForm({ ...registerForm, role: e.target.value })} required
                                className="w-full pl-8 pr-6 py-2 bg-white border border-slate-300 rounded-lg focus:border-indigo-500 outline-none text-[10px] font-black uppercase appearance-none text-slate-900">
