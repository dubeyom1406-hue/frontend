...
const WalletControl = ({ initialTab = 'credit' }) => {
    ...
    const loadData = async () => {
        try {
            // Fetch fund requests
            const reqData = await walletService.getFundRequests();
            if (reqData.success) setRequests(reqData.requests);

            // Fetch user wallets overview
            const walData = await walletService.getUserWallets();
    ...
    const handleAction = async (e) => {
        ...
        setMessage(null);
        try {
            const data = await walletService.performAction(activeTab, formData);
            if (data.success) {
    ...
    const handleApprove = async (id, status) => {
        try {
            const data = status === 'APPROVED' 
                ? await walletService.approveRequest(id, 'Processed by Admin')
                : await walletService.rejectRequest(id, 'Processed by Admin');
