    ...
    const loadData = async () => {
        ...
        try {
            ...
            if (response && response.users) {
                const distsOnly = response.users.filter(u => 
                    u.role?.toUpperCase() === 'DISTRIBUTOR' || u.role?.toUpperCase() === 'SUPER_DISTRIBUTOR'
                );
                // Map fields if necessary (mapping backend id to distributor id)
                const mappedDists = distsOnly.map(d => ({
                    ...d,
                    id: d.id, // Backend returns numeric id
                    name: d.full_name || d.fullName || d.username,
                    mobile: d.mobile || d.phone,
                    wallet: { balance: d.balance || '0.00' }
                }));
                setDistributors(mappedDists);
            }
        } catch (err) {
