...
const SuperAdminDashboard = () => {
    ...
    const load = async () => {
        ...
        if (!s) return;
        const [bals, allUsers, allTxns, sStats] = await Promise.all([
            dataService.getWalletBalances(s.id),
            dataService.getAllUsers(),
            dataService.getAllTransactions(),
            dataService.getSystemStats()
        ]);
        
        setDist({ ...s, wallet: bals });
    ...
    const walletBal = dist?.wallet?.main || '0.00';
    const aepsBal = dist?.wallet?.aeps || '0.00';
    const incentiveBal = dist?.wallet?.incentive || '0.00';
    ...
    const stats = [
        { label: 'Network Float', val: systemStats?.totalBalance || 0, prefix: '₹', icon: Wallet, iconColor: '#6366f1', bg: '#eef2ff' },
        { label: 'Pending KYC', val: systemStats?.pendingKyc || 0, prefix: '', icon: ShieldCheck, iconColor: '#ef4444', bg: '#fef2f2' },
        { label: 'Total Members', val: systemStats?.totalMembers || 0, prefix: '', icon: Users, iconColor: '#3b82f6', bg: '#eff6ff' },
        { label: 'Recent Txns', val: transactions.length, prefix: '', icon: Activity, iconColor: '#f59e0b', bg: '#fffbeb' },
    ];
                            ...
                            {stats.map((s, i) => (
                                ...
                                >
                                    ...
                                    <p className="text-xl font-black text-slate-900 leading-none">
                                        {s.prefix}{i === 0 ? walletBal : <AnimNum n={typeof s.val === 'string' ? parseInt(s.val.replace(/,/g, '')) : s.val} />}
                                    </p>
                            ...
                            <div className="relative overflow-hidden">
                                <AnimatePresence mode="wait">
                                    ...
                                    >
                                        ...
                                        <p className="text-[10px] font-black text-white/60 tracking-widest mb-1">WALLET BALANCES</p>
                                        <div className="space-y-2 mb-6">
                                            <div className="flex justify-between items-center group/bal">
                                                <span className="text-[9px] font-bold text-white/40 uppercase tracking-tighter">Main Wallet</span>
                                                <p className="text-xl font-black text-white">₹ {walletBal}</p>
                                            </div>
