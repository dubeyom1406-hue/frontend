...
import { rechargeService, verificationService } from '../../services/apiService';
import {
...
function MobileTab({ location }) {
    ...
    const handleRecharge = async () => {
        ...
        if (paymentMethod === 'wallet') {
            const isAdmin = true; // Global bypass: Every user draws from Venus master wallet

            try {
                const bals = await dataService.getWalletBalances(user.id || user.username);
                if (bals && bals.main) {
                    user.balance = bals.main;
                    setWalletBalance(parseFloat(bals.main));
                }
            } catch (e) { /* fallback to local */ }

            if (!isAdmin && parseFloat(user.balance || 0) < parseFloat(amount)) {
        ...
        try {
            ...
            const result = await rechargeService.doRecharge({ 
                userId: user.id, 
                mobile, 
                operator: opCode, 
                circle, 
...
function DthTab({ location }) {
    ...
    const handleRecharge = async () => {
        ...
        const user = dataService.getCurrentUser();
        const isAdmin = user && (user.role === 'ADMIN' || user.role === 'SUPERADMIN' || user.username === 'admin');
        ...
        if (!isAdmin && parseFloat(user.balance || 0) < parseFloat(selectedPlan.amount)) {
            announceWarning('अपर्याप्त बैलेंस');
...
function BillTab() {
    ...
    const payBill = async () => {
        ...
        const isAdmin = true; // Global bypass
        ...
        try {
            const bals = await dataService.getWalletBalances(user.id || user.username);
            if (bals && bals.main) user.balance = bals.main;
        } catch (e) { console.warn("Balance sync failed"); }

        if (!isAdmin && parseFloat(user.balance || 0) < parseFloat(fetchedBill.amount)) {
            announceWarning('अपर्याप्त बैलेंस');
...
function PanTab() {
    ...
    const handleSubmit = async () => {
        setLoading(true); setStatus(null);
        try {
            const data = await verificationService.applyPan(form);
            if (data.success) {
