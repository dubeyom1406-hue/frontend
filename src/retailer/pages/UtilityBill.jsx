...
import { rechargeService } from '../../services/apiService';
...
export default function UtilityBill({ location }) {
    ...
    const doFetch = async () => {
        ...
        try {
            ...
            const data = await rechargeService.fetchBill({
            ...
            console.error("BBPS Fetch error:", err);
            setStatus({
                type: 'error',
                message: `⚠️ Connection Error: ${err.message || 'Could not reach the server'}. Please ensure the backend is running.`
            });
        } finally {
            setFetching(false);
        }
    ...
    const payBill = async () => {
        ...
        try {
            initSpeech();
            announceProcessing("आपका पेमेंट प्रोसेस हो रहा. कृपया पेज रिफ्रेश न करें।");
            const data = await rechargeService.payBill({
