...
import {
    ...
    Hash, Building2, ChevronDown, Loader2, X, ShieldCheck, History, Database, Users
} from 'lucide-react';
...
const UserSelector = ({ users, value, onChange, placeholder }) => {
    ...
    localUsers.forEach(lu => {
        if (!allUsers.find(au => au.username === lu.username || au.id == lu.id || (lu.mobile && au.mobile === lu.mobile))) {
            allUsers.push({ ...lu, isLocal: true, name: lu.name || lu.fullName });
        }
    });

    const selected = allUsers.find(u => u.username === value || u.userId == value || u.id == value || (value && u.mobile == value));
    const filtered = allUsers.filter(u => {
        const searchName = (u.name || u.fullName || u.username || "").toLowerCase();
        const searchUsername = (u.username || "").toLowerCase();
        const searchMobile = (u.mobile || "");
        const s = search.toLowerCase();
        return searchName.includes(s) || searchUsername.includes(s) || searchMobile.includes(s);
    });
    return (
                        ...
                        <div className="max-h-48 overflow-y-auto">
                            {filtered.length === 0 ? (
                                <div className="p-10 text-center">
                                    ...
                                    <p className="text-[11px] font-black text-slate-800 uppercase tracking-tight mb-1">{search ? "No Matches Found" : "Database Empty"}</p>
                                    <p className="text-[9px] font-bold text-slate-400 uppercase tracking-widest leading-relaxed mb-5">
                                        Members might be in Cache but not Live.
                                    </p>
                                    {!search && (
                                        <button type="button" onClick={() => setOpen(false)}
                                            className="px-5 py-2.5 bg-indigo-600 text-white rounded-xl text-[9px] font-black uppercase tracking-widest hover:scale-105 active:scale-95 transition-all shadow-lg shadow-indigo-100">
                                            Sync in Members Tab
                                        </button>
                                    )}
                                </div>
                                ...
                                <button key={i} type="button"
                                    onClick={() => { onChange(u.isLocal ? (u.id || u.username) : u.id); setOpen(false); setSearch(''); }}
                                    className={`w-full flex items-center gap-3 px-4 py-3 hover:bg-indigo-50 transition-colors text-left ${u.isLocal ? 'bg-slate-50/50' : ''}`}>
                                    <div className={`w-7 h-7 rounded-lg flex items-center justify-center text-xs font-black shrink-0 ${u.isLocal ? 'bg-slate-200 text-slate-500' : 'bg-indigo-100 text-indigo-600'}`}>
                                        {(u.name || u.fullName)?.charAt(0) || 'U'}
                                    </div>
                                    <div>
                                        <div className="flex items-center gap-2">
                                            <p className="text-xs font-bold text-slate-800">{u.name || u.fullName}</p>
                                            {u.isLocal && <span className="px-1 py-0.5 bg-amber-100 text-amber-700 text-[7px] font-black rounded uppercase tracking-tighter shadow-sm border border-amber-200">LOCAL CACHE</span>}
                                        </div>
                                        <div className="flex items-center gap-2 mt-0.5">
                                            <span className={`px-1.5 py-0.5 rounded text-[8px] font-black uppercase tracking-tighter ${
                                                u.isLocal ? 'bg-slate-100 text-slate-500' :
                                                u.role === 'SUPERADMIN' ? 'bg-indigo-100 text-indigo-600' :
                                                u.role === 'DISTRIBUTOR' ? 'bg-purple-100 text-purple-600' :
                                                'bg-blue-100 text-blue-600'
                                            }`}>
                                                {u.role ||'RETAILER'}
                                            </span>
                                            <p className="text-[10px] text-slate-400 font-medium">{u.mobile} · Balance: <span className="text-emerald-600 font-bold">₹{(u.balance || u.availableBalance || 0).toLocaleString('en-IN')}</span></p>
                                        </div>
                                    </div>
                                </button>
                            ))}
                            ...
                            {filtered.length === 0 && (
                                <div className="p-12 text-center">
                                    ...
                                    <p className="text-sm font-black text-slate-800 uppercase tracking-tight mb-2">
                                        {search ? `No results for "${search}"` : "Live Database Empty"}
                                    </p>
                                    <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest leading-relaxed mb-6">
                                        {dataService.getData().users?.length > 0
                                            ? "Members exist in cache but need to be pushed to the Live Database."
                                            : "Go to 'Members' to add your retailers and distributors first."}
                                    </p>
                                    {dataService.getData().users?.length > 0 && !search && (
                                        <button
                                            onClick={() => window.location.hash = '#Members'} // This is a hint for the user
                                            className="inline-flex items-center gap-2 px-6 py-3 bg-indigo-600 text-white rounded-xl text-[10px] font-black uppercase tracking-widest shadow-lg shadow-indigo-100 hover:scale-105 active:scale-95 transition-all"
                                        >
                                            <Database size={14} /> Sync from Members Tab
                                        </button>
                                    )}
                                </div>
                            )}
                    ...
                    <div className="space-y-1.5">
                        ...
                        <div className="relative">
                            ...
                            <input type="number" min="1" step="0.01" value={amount} onChange={e => setAmount(e.target.value)}
                                placeholder="0.00"
                                className={`w-full pl-8 pr-4 py-3 bg-slate-50 border-2 border-slate-200 rounded-xl text-base font-black text-slate-900 outline-none focus:border-${color}-500 transition-all`} />
                        </div>
                    </div>
                ...
                <form onSubmit={handleSubmit} className="space-y-5">
                    ...
                    <div className="space-y-1.5">
                        ...
                        <div className="relative">
                            ...
                            <input type="number" min="1" step="0.01" value={amount} onChange={e => setAmount(e.target.value)}
                                placeholder="0.00"
                                className={`w-full pl-8 pr-4 py-3 bg-slate-50 border-2 border-slate-200 rounded-xl text-base font-black text-slate-900 outline-none transition-all ${isLock ? 'focus:border-purple-500' : 'focus:border-cyan-500'}`} />
                        </div>
                    </div>
                    ...
                    <form onSubmit={handleSubmit} className="space-y-4">
                        ...
                        <div className="grid grid-cols-2 gap-4">
                            <div className="space-y-1.5">
                                ...
                                <input type="number" value={amount} onChange={e => setAmount(e.target.value)} className="w-full px-4 py-3 bg-slate-50 border-2 border-slate-200 rounded-xl text-sm font-black text-slate-900 outline-none focus:border-pink-500 transition-all" placeholder="0.00" />
                            </div>
                            <div className="space-y-1.5">
