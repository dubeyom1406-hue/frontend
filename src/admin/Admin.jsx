...
import {
    ...
    Camera, Eye, IndianRupee, ChevronRight, Wallet, TrendingUp, History as HistoryIcon, ArrowRight,
    Building2, UserPlus, UserMinus, ShieldCheck, Link2, Copy, Crown, ChevronDown, Mail, MapPin, Search, Smartphone, Clock as ClockIcon, LayoutGrid, User as UserIcon, Activity as ActivityIcon, Database, CloudUpload, Landmark as LandmarkIcon, Headphones
} from 'lucide-react';
...
const Admin = () => {
    ...
    useEffect(() => {
        ...
        // --- Race-condition fix: check localStorage if currentUser state is currently null ---
        const savedUserStr = localStorage.getItem('rupiksha_user');
        let userForGuard = currentUser;
        if (!userForGuard && savedUserStr) {
            try { userForGuard = JSON.parse(savedUserStr); } catch(e) {}
        }

        const ADMIN_ROLES = ['ADMIN', 'SUPERADMIN', 'NATIONAL_HEADER', 'STATE_HEADER', 'REGIONAL_HEADER', 'EMPLOYEE'];
        const hasAdminRole = userForGuard && ADMIN_ROLES.includes(String(userForGuard.role).toUpperCase());
        
        if (!isAuth && !hasAdminRole) {
            navigate('/admin-login', { replace: true });
        } else if (hasAdminRole && !isAuth) {
            // Sync session storage if we have a valid admin role
    ...
    const handleUnifiedAddMember = async (e) => {
        ...
        try {
            // Use apiService / mock logic directly here instead of raw fetch
            ...
            const resData = await adminService.createUser(payload);
            ...
            if (resData.success) {
                ...
                alert(resData.message || resData.error || "Registry Failed. User might already exist.");
            }
        } catch (err) {
    ...
    const refreshData = async () => {
        try {
            ...
            // WE MARK THEM AS REMOTE TO DIFFERENTIATE FROM CACHED LOCAL STORAGE
            const retailers = allUsers.filter(u => u.role === 'RETAILER').map(u => ({ ...u, isRemote: true }));
            const dists = allUsers.filter(u => u.role === 'DISTRIBUTOR').map(u => ({ ...u, isRemote: true }));
            const sas = allUsers.filter(u => u.role === 'SUPER_DISTRIBUTOR').map(u => ({ ...u, isRemote: true }));

            const currentData = dataService.getData();
            setData({ ...currentData, users: retailers });
            setDistributors(dists);
            setSuperadmins(sas);
            setStatus({ type: 'success', message: 'Data synced with live server' });
    ...
    const handleSyncMembersToCloud = async (targetUser = null) => {
        if (!targetUser && !window.confirm("This will push all your current members to the live database. Continue?")) return;
        setIsSyncing(true);
        const token = localStorage.getItem('rupiksha_token');
        
        // Use either the single target user or the full local list
        const allToSync = targetUser ? [targetUser] : [...(data.users || []), ...(distributors || []), ...(superadmins || [])];
        
        let count = 0;
        for (const m of allToSync) {
            console.log(`Attempting sync for: ${m.username || m.mobile}`);
            try {
                const res = await fetch(`${BACKEND_URL}/admin/add-user`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${token}` },
                    body: JSON.stringify({
                        username: m.username || m.mobile,
                        password: m.password || '123456',
                        pin: m.pin || '1122',
                        fullName: m.name || m.fullName,
                        phone: m.mobile,
                        email: m.email,
                        address: m.address || (m.city ? m.city + ', ' : '') + (m.state || 'Bihar'),
                        role: m.role || (m.memberType === 'Retailer' ? 'RETAILER' : m.memberType === 'Distributor' ? 'DISTRIBUTOR' : 'SUPER_DISTRIBUTOR'),
                        territory: m.state || 'Bihar',
                        shopName: m.businessName || m.business_name || 'N/A'
                    })
                });
                
                if (res.ok || res.status === 400) { // Success or already exists
                    if (res.ok) count++;
                } else {
        ...
        setIsSyncing(false);
        if (targetUser) {
            if (count > 0) alert("Member successfully pushed to live database!");
            else alert("Member is already live or sync failed. Check console for details.");
        } else {
            alert(`Sync process complete. ${count} new members were added to the proper database!`);
        }
        refreshData();
    };
        ...
        return (
            <div className="space-y-2">
                ...
                <div
                    ...
                    className="relative group border-2 border-dashed border-slate-200 rounded-xl p-4 transition-all hover:border-blue-400 bg-slate-50/50"
                >
                    {value ? (
                        ...
                        <label className="flex flex-col items-center justify-center py-4 cursor-pointer">
                            <div className="p-3 bg-white rounded-full shadow-sm text-slate-400 group-hover:text-blue-500 group-hover:scale-110 transition-all mb-2">
                                <Upload size={20} />
                            </div>
                            <span className="text-[10px] font-bold text-slate-400 uppercase group-hover:text-blue-600">Click or Drag Image</span>
                            <input type="file" className="hidden" accept="image/*" onChange={(e) => handleFile(e.target.files[0])} />
                        </label>
                    )}
                </div>
    ...
    const DashboardEditor = () => (
        <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4">
            ...
            <div className="p-6 bg-white rounded-2xl shadow-sm border border-slate-100">
                <h3 className="font-bold text-slate-700 flex items-center gap-2 mb-6"><Zap size={18} className="text-blue-500" /> Quick Action Cards</h3>
            ...
            <div className="bg-white rounded-3xl p-6 shadow-sm border border-slate-100 space-y-5">
                ...
                <div className="space-y-4">
                    {['today', 'weekly', 'monthly', 'debit'].map(key => (
                        <div key={key} className="p-4 bg-slate-50/50 rounded-2xl border border-slate-100 space-y-3 hover:border-blue-200 transition-colors group">
                            <div className="flex justify-between items-center">
                            ...
                            <input type="text" className="w-full px-3 py-2 bg-white border-2 border-slate-200 rounded-xl text-[10px] uppercase font-black text-blue-600 tracking-wider outline-none focus:border-emerald-500"
    ...
    const ServicesEditor = () => (
        <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4">
            ...
            {(data.services || []).filter(s => s && s.items).map((cat, catIdx) => (
                        ...
                        {data.loginLogs?.map((log, i) => (
                            <tr key={i} className="hover:bg-slate-50/50 transition-colors">
                                ...
                                <td className="px-8 py-4">
                                    <span className={`px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-tight ${log.status === 'success' ? 'bg-blue-50 text-blue-600' : 'bg-rose-50 text-rose-600'}`}>
    ...
    const ApprovalModal = () => (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm">
            ...
            >
                <div className="flex justify-between items-center mb-4">
                    <h3 className="text-xl font-bold text-slate-800 flex items-center gap-2">
                        <CheckCircle2 size={20} className="text-blue-500" /> Approve Retailer
                    </h3>
                    <button onClick={() => setShowApprovalModal(false)} className="text-slate-400 hover:text-slate-600"><X size={24} /></button>
                </div>

                <div className="flex items-center gap-2 bg-blue-50 border border-blue-200 rounded-xl px-4 py-2 mb-5">
                    <span className="text-[9px] font-black text-blue-600 uppercase tracking-widest">📧 Auto-email to:</span>
                    <span className="text-xs font-black text-blue-700 ml-auto">{selectedUser?.email || 'No email on record'}</span>
                </div>
                ...
                <div className="space-y-4">
                    ...
                    <div className="grid grid-cols-2 gap-3">
                        <div>
                            <label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest block mb-1">Password <span className="text-blue-500">(editable)</span></label>
                            <input type="text"
                        ...
                        <div>
                            <label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest block mb-1">Security PIN <span className="text-blue-500">(editable)</span></label>
                    ...
                    <div>
                        <label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest block mb-1">Party Code <span className="text-blue-500">(editable)</span></label>
                        <input type="text"
                    ...
                    <button onClick={submitApproval}
                        className="w-full bg-blue-500 text-white font-black py-4 rounded-xl shadow-lg shadow-emerald-500/30 hover:bg-emerald-400 transition-all active:scale-95 uppercase tracking-widest text-xs flex items-center justify-center gap-2"
                    >
    ...
    const RetailersTable = () => (
        <div className="bg-white rounded-[2rem] shadow-sm border border-slate-100 overflow-hidden animate-in fade-in slide-in-from-bottom-4">
            <div className="px-8 py-6 border-b border-slate-50 flex justify-between items-center bg-slate-50/30">
                <div className="flex items-center gap-4">
                    <div className="w-11 h-11 rounded-2xl bg-blue-500 flex items-center justify-center text-white shadow-lg shadow-emerald-500/20">
                        <Users size={20} />
                    </div>
                    <div>
                ...
                <span className="px-4 py-2 bg-blue-50 text-blue-600 rounded-2xl text-[11px] font-black tracking-widest uppercase">{(data.users || []).length} TOTAL AGENTS</span>
            </div>
            <div className="overflow-x-auto">
                    ...
                    <tbody className="divide-y divide-slate-50 text-xs">
                        {(data.users || []).map((user, idx) => (
                            user && <tr key={idx} className="hover:bg-slate-50/50 transition-colors group">
                                ...
                                <td className="px-8 py-4">
                                    <span className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-[9px] font-black uppercase tracking-tight ${user.status === 'Approved' ? 'bg-blue-50 text-blue-600' : 'bg-rose-50 text-rose-600'}`}>
                                        <div className={`w-1.5 h-1.5 rounded-full ${user.status === 'Approved' ? 'bg-blue-500 animate-pulse' : 'bg-rose-500'}`} />
                                        {user.status || 'PENDING'}
                                    </span>
                                </td>
                                <td className="px-8 py-4">
                                ...
                                <td className="px-8 py-4 text-right">
                                    ...
                                    <p className="text-[8px] font-black text-blue-500 uppercase tracking-widest mt-1">Real-time Balance</p>
                                </td>
                                ...
                                <td className="px-8 py-4 text-center">
                                    <div className="flex justify-center gap-2">
                                        ...
                                        <button onClick={() => handleLoginAsRetailer(user)} className="p-2 bg-blue-50 text-blue-600 rounded-lg hover:bg-blue-600 hover:text-white transition-all shadow-sm" title="Impersonate"><ArrowRight size={15} /></button>
                            ...
                            <tr key={idx} className="hover:bg-rose-50/10 transition-colors">
                                ...
                                <td className="px-8 py-4">
                                    <div className="flex justify-center gap-3">
                                        <button onClick={() => handleRestoreUser(t.username)} className="flex items-center gap-2 px-6 py-2.5 bg-blue-50 text-blue-600 rounded-2xl hover:bg-blue-600 hover:text-white transition-all text-[10px] font-black uppercase tracking-widest shadow-sm">
                    ...
                    <div className="p-10 space-y-10 text-center">
                        ...
                        <div className="space-y-4">
                            <button
                                ...
                                className="w-full bg-blue-500 text-white font-black py-5 rounded-2xl text-[11px] uppercase tracking-[0.25em] shadow-2xl active:scale-95 transition-all disabled:opacity-50"
                            >
    ...
    const renderCredentialSharerModal = () => {
        ...
        return (
            <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 bg-slate-900/80 backdrop-blur-md">
                ...
                >
                    <div className="relative">
                        <div className="w-24 h-24 bg-blue-50 text-blue-500 rounded-3xl flex items-center justify-center mx-auto border-4 border-blue-100/50 shadow-inner">
                                    ...
                                    <button
                                        ...
                                        className={`px-6 py-2.5 rounded-xl font-black text-[10px] uppercase tracking-widest transition-all shadow-sm active:scale-95
                                            ${isAssigned
                                                ...
                                                : 'bg-blue-500 text-white hover:bg-blue-600 shadow-emerald-500/20'}`}
                                    >
                    ...
                    <div className="p-10 space-y-10 text-center">
                        ...
                        <div className="space-y-4">
                            <button
                                ...
                                className="w-full bg-blue-500 text-white font-black py-5 rounded-[2rem] text-[11px] uppercase tracking-[0.25em] shadow-2xl shadow-emerald-500/20 active:scale-95 transition-all disabled:opacity-50"
                            >
                    ...
                    <div className="p-10 border-b border-slate-50 flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
                        <div>
                            <h3 className="text-2xl font-black text-slate-800 flex items-center gap-3 italic uppercase tracking-tight">
                                <Building2 size={28} className="text-blue-500" /> Distributor_Network
                            </h3>
                        ...
                        <div className="flex items-center gap-4">
                            ...
                            <div className="px-4 py-4 bg-blue-50 border border-blue-100 rounded-2xl text-center min-w-[120px]">
                                <span className="block text-[8px] font-black text-blue-400 uppercase tracking-widest mb-1">Network_Size</span>
                                <span className="text-xl font-black text-blue-600 leading-none">{approved.length} ACCOUNTS</span>
                            </div>
                        </div>
                    </div>
                    <div className="overflow-x-auto">
                                        ...
                                        <td className="px-10 py-8">
                                            <div className="flex items-center gap-5">
                                                <div className="relative">
                                                    ...
                                                    <div className="absolute -bottom-1 -right-1 w-6 h-6 bg-blue-500 rounded-full border-4 border-white flex items-center justify-center text-white">
                                                        <ShieldCheck size={10} />
                                                    </div>
                                                </div>
                                                <div>
                                        ...
                                        <td className="px-10 py-8">
                                            <div className="flex items-center justify-end gap-2.5">
                                                ...
                                                <button
                                                    ...
                                                    className="flex items-center gap-2 bg-blue-50 text-blue-600 px-4 py-3 rounded-xl border border-blue-100 hover:bg-blue-600 hover:text-white transition-all group/btn"
                                                    title="Simulate Node"
                                                >
                    ...
                    <div className="fixed inset-0 z-[110] bg-black/60 backdrop-blur-sm flex items-center justify-center p-4">
                        <div className="bg-white rounded-[2rem] w-full max-w-4xl max-h-[90vh] overflow-hidden flex flex-col">
                            ...
                            <div className="p-6 border-t bg-white flex justify-end gap-3">
                                ...
                                <button onClick={() => { handleApproveKyc(viewingDocs.username, viewingDocs.merchant_id); setViewingDocs(null); }} className="px-8 py-3 bg-blue-600 text-white rounded-2xl text-[11px] font-black uppercase tracking-widest hover:bg-emerald-700 shadow-lg shadow-emerald-500/30 transition-all">Approve KYC</button>
                            </div>
                        </div>
                    </div>
                )}
...
;
    ...
    const MANAGEMENT_NAV = [
        { id: 'EmployeeManager', icon: Users, label: 'Employee Management' },
        { id: 'Landing Content', icon: FileText, label: 'Website Management' },
        { id: 'AllMembers', icon: ShieldCheck, label: 'Members/Plans' },
    ...
    const PAYOUT_NAV = [
        { id: 'Payout-Account', icon: ShieldCheck, label: 'Account Approval' },
        { id: 'Payout-Pending', icon: ClockIcon, label: 'Pending Account Approval' },
    ];

    const SERVICES_NAV = [
        { id: 'List-Services', icon: LayoutGrid, label: 'List Services' },
        { id: 'Add-Services', icon: Plus, label: 'Add Services' },
    ];

    const SETTINGS_NAV = [
        { id: 'Settings-Logs', icon: FileText, label: 'Admin Logs' },
        { id: 'Settings-Main', icon: Settings, label: 'Main Settings' },
        { id: 'Settings-Package', icon: Package, label: 'Package' },
        { id: 'Settings-Commission', icon: LandmarkIcon, label: 'Commission & Charge' },
    ];

    const SUPPORT_NAV = [
        { id: 'Support-Pending', icon: ClockIcon, label: 'Pending Support Ticket' },
        { id: 'Support-Resolved', icon: CheckCircle2, label: 'Resolved Tickets' },
    ];

    const TRANSACTIONS_NAV = [
        { id: 'Txn-BBPS', icon: LayoutGrid, label: 'BBPS Transactions' },
        { id: 'Txn-AEPS', icon: Smartphone, label: 'AEPS Transactions' },
        { id: 'Txn-DMT', icon: LandmarkIcon, label: 'DMT Transactions' },
        { id: 'Txn-Payout', icon: IndianRupee, label: 'Payout Transactions' },
        { id: 'Txn-Wallet', icon: Wallet, label: 'Wallet Transfer' },
    ];
    ...
    const activeLabel = ANALYTICS_NAV.find(n => n.id === activeSection)?.label
        ...
        || PAYOUT_NAV.find(n => n.id === activeSection)?.label
        || SERVICES_NAV.find(n => n.id === activeSection)?.label
        || SETTINGS_NAV.find(n => n.id === activeSection)?.label
        || SUPPORT_NAV.find(n => n.id === activeSection)?.label
        || MANAGEMENT_NAV.find(n => n.id === activeSection)?.label
        || TRANSACTIONS_NAV.find(n => n.id === activeSection)?.label
    ...
    return (
        <div className="h-screen flex relative overflow-hidden bg-[#fcfcf7]" style={{ fontFamily: "'Inter',sans-serif" }}>
            ...
            <aside className={`fixed lg:relative !w-72 bg-white border-r border-[#f1f5f9] flex flex-col h-screen z-50 transition-all duration-300 ${showMobileSidebar ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'} shrink-0`}>
                ...
                {/* Logo area with premium floating vibe */}
                <div className="flex items-center gap-4 px-10 py-10 shrink-0">
                    <div className="w-10 h-10 bg-black rounded-2xl flex items-center justify-center text-white shadow-xl shadow-black/20 transform -rotate-12">
                        <Zap size={20} fill="currentColor" />
                    </div>
                    <h1 className="text-2xl font-black text-slate-900 tracking-tighter uppercase italic">RUPIKSHA</h1>
                </div>

                {/* Navigation */}
                <nav className="flex-1 overflow-y-auto py-2 px-6 space-y-1 scrollbar-hide">
                    
                    {/* Analytics Section */}
                    <div className="space-y-1.5 pt-4">
                        <p className="text-[10px] font-black text-[#94a3b8] uppercase tracking-[0.2em] mb-3 ml-4">Analytics Hub</p>
                        {ANALYTICS_NAV.map(item => {
                            const isActive = activeSection === item.id;
                            return (
                                <button key={item.id}
                                    onClick={() => { setActiveSection(item.id); setShowMobileSidebar(false); }}
                                    className={`w-full flex items-center justify-between px-5 py-3.5 rounded-full transition-all duration-300 ${isActive ? 'bg-white border shadow-lg border-slate-100/50 scale-[1.02]' : 'hover:bg-slate-50 opacity-75 hover:opacity-100'}`}>
                                    <div className="flex items-center gap-4">
                                         <item.icon size={18} className={isActive ? 'text-blue-600' : 'text-slate-400'} />
                                         <span className={`text-[14px] font-extrabold ${isActive ? 'text-slate-900' : 'text-[#64748b]'}`}>{item.label}</span>
                                    </div>
                                </button>
                            );
                        })}
                    ...
                    <div className="space-y-1">
                        <button 
                            onClick={() => setExpandedNav(expandedNav === 'Transactions' ? null : 'Transactions')}
                            className={`w-full flex items-center justify-between px-5 py-3.5 rounded-full transition-all duration-300 ${activeSection.startsWith('Txn-') ? 'bg-white border shadow-lg border-slate-100/50 scale-[1.02]' : 'hover:bg-slate-50 opacity-75 hover:opacity-100'}`}>
                            <div className="flex items-center gap-4">
                                 <HistoryIcon size={18} className={activeSection.startsWith('Txn-') ? 'text-blue-600' : 'text-slate-400'} />
                                 <span className={`text-[14px] font-extrabold ${activeSection.startsWith('Txn-') ? 'text-slate-900' : 'text-[#64748b]'}`}>Transaction Hub</span>
                            </div>
                            <ChevronDown size={14} className={`text-slate-400 transition-transform duration-300 ${expandedNav === 'Transactions' ? 'rotate-180' : ''}`} />
                        </button>
                        <AnimatePresence>
                            {expandedNav === 'Transactions' && (
                                <motion.div 
                                    initial={{ height: 0, opacity: 0 }} 
                                    animate={{ height: 'auto', opacity: 1 }} 
                                    exit={{ height: 0, opacity: 0 }}
                                    className="overflow-hidden bg-white/50 backdrop-blur-sm rounded-2xl border border-slate-200/40 ml-4 mt-1">
                                    {TRANSACTIONS_NAV.map(item => {
                                        const isActive = activeSection === item.id;
                                        return (
                                            <button key={item.id}
                                                onClick={() => { setActiveSection(item.id); setShowMobileSidebar(false); }}
                                                className={`w-full flex items-center gap-4 px-6 py-3 transition-all duration-300 rounded-full mb-1 text-[13px] font-black ${isActive ? 'text-blue-600 bg-blue-50/50 shadow-sm border border-blue-100/30' : 'text-[#64748b] hover:text-[#18181b] hover:bg-slate-50'}`}>
                                                <item.icon size={15} className={isActive ? 'text-blue-600' : 'text-[#94a3b8]'} />
                                                {item.label}
                                            </button>
                                        );
                                    })}
                    ...
                    {/* Operations Section */}
                    <div className="space-y-1.5">
                        <p className="text-[10px] font-black text-[#94a3b8] uppercase tracking-[0.2em] mb-3 ml-4">Core Actions</p>
                        {OPERATIONS_NAV.map(item => {
                            ...
                            return (
                                <button key={item.id}
                                    ...
                                    {item.badge && (
                                        <span className="bg-blue-50 text-blue-600 px-2.5 py-1 rounded-lg text-[9px] font-black border border-blue-100">{item.badge}</span>
                                    )}
                                </button>
                            );
                        })}
                    ...
                    <div className="space-y-1">
                        <button 
                            onClick={() => setExpandedNav(expandedNav === 'Payout' ? null : 'Payout')}
                            className={`w-full flex items-center justify-between px-5 py-3.5 rounded-full transition-all duration-300 ${activeSection.startsWith('Payout') ? 'bg-white border shadow-lg border-slate-100/50 scale-[1.02]' : 'hover:bg-slate-50 opacity-75 hover:opacity-100'}`}>
                            <div className="flex items-center gap-4">
                                 <IndianRupee size={18} className={activeSection.startsWith('Payout') ? 'text-blue-600' : 'text-slate-400'} />
                                 <span className={`text-[14px] font-extrabold ${activeSection.startsWith('Payout') ? 'text-slate-900' : 'text-[#64748b]'}`}>Settlement Hub</span>
                            </div>
                            <ChevronDown size={14} className={`text-slate-400 transition-transform duration-300 ${expandedNav === 'Payout' ? 'rotate-180' : ''}`} />
                        </button>
                        <AnimatePresence>
                            {expandedNav === 'Payout' && (
                                <motion.div 
                    ...
                    <div className="space-y-1">
                        <button 
                            onClick={() => setExpandedNav(expandedNav === 'Management' ? null : 'Management')}
                            className={`w-full flex items-center justify-between px-3 py-2.5 rounded-2xl transition-all duration-200 hover:bg-slate-50 ${['EmployeeManager', 'Landing Content'].includes(activeSection) ? 'bg-slate-50 border border-slate-100' : ''}`}>
                            <div className="flex items-center gap-4">
                                 <Settings size={18} className={['EmployeeManager', 'Landing Content'].includes(activeSection) ? 'text-[#18181b]' : 'text-[#94a3b8]'} />
                                 <span className={`text-[14px] font-bold ${['EmployeeManager', 'Landing Content'].includes(activeSection) ? 'text-[#18181b]' : 'text-[#64748b]'}`}>Management</span>
                            </div>
                            <ChevronDown size={14} className={`text-slate-400 transition-transform duration-200 ${expandedNav === 'Management' ? 'rotate-180' : ''}`} />
                        </button>
                        <AnimatePresence>
                            {expandedNav === 'Management' && (
                                <motion.div 
                                    initial={{ height: 0, opacity: 0 }} 
                                    animate={{ height: 'auto', opacity: 1 }} 
                                    exit={{ height: 0, opacity: 0 }}
                    ...
                    <div className="space-y-1">
                        <button 
                            onClick={() => setExpandedNav(expandedNav === 'OtherServices' ? null : 'OtherServices')}
                            className={`w-full flex items-center justify-between px-5 py-3.5 rounded-full transition-all duration-300 ${activeSection.startsWith('List-') || activeSection.startsWith('Add-') ? 'bg-white border shadow-lg border-slate-100/50 scale-[1.02]' : 'hover:bg-slate-50 opacity-75 hover:opacity-100'}`}>
                            <div className="flex items-center gap-4">
                                 <Plus size={18} className={activeSection.startsWith('List-') || activeSection.startsWith('Add-') ? 'text-blue-600' : 'text-slate-400'} />
                                 <span className={`text-[14px] font-extrabold ${activeSection.startsWith('List-') || activeSection.startsWith('Add-') ? 'text-slate-900' : 'text-[#64748b]'}`}>Platform Services</span>
                            </div>
                            <ChevronDown size={14} className={`text-slate-400 transition-transform duration-300 ${expandedNav === 'OtherServices' ? 'rotate-180' : ''}`} />
                        </button>
                        <AnimatePresence>
                            {expandedNav === 'OtherServices' && (
                                <motion.div 
                                    initial={{ height: 0, opacity: 0 }} 
                                    animate={{ height: 'auto', opacity: 1 }} 
                                    exit={{ height: 0, opacity: 0 }}
                    ...
                    <div className="space-y-1">
                        <button 
                            onClick={() => setExpandedNav(expandedNav === 'Settings' ? null : 'Settings')}
                            className={`w-full flex items-center justify-between px-5 py-3.5 rounded-full transition-all duration-300 ${activeSection.startsWith('Settings-') ? 'bg-white border shadow-lg border-slate-100/50 scale-[1.02]' : 'hover:bg-slate-50 opacity-75 hover:opacity-100'}`}>
                            <div className="flex items-center gap-4">
                                 <Settings size={18} className={activeSection.startsWith('Settings-') ? 'text-blue-600' : 'text-slate-400'} />
                                 <span className={`text-[14px] font-extrabold ${activeSection.startsWith('Settings-') ? 'text-slate-900' : 'text-[#64748b]'}`}>Control Panel</span>
                            </div>
                            <ChevronDown size={14} className={`text-slate-400 transition-transform duration-300 ${expandedNav === 'Settings' ? 'rotate-180' : ''}`} />
                        </button>
                        <AnimatePresence>
                    ...
                    <div className="space-y-1">
                        <button 
                            onClick={() => setExpandedNav(expandedNav === 'Support' ? null : 'Support')}
                            className={`w-full flex items-center justify-between px-5 py-3.5 rounded-full transition-all duration-300 ${activeSection.startsWith('Support-') ? 'bg-white border shadow-lg border-slate-100/50 scale-[1.02]' : 'hover:bg-slate-50 opacity-75 hover:opacity-100'}`}>
                            <div className="flex items-center gap-4">
                                 <Headphones size={18} className={activeSection.startsWith('Support-') ? 'text-blue-600' : 'text-slate-400'} />
                                 <span className={`text-[14px] font-extrabold ${activeSection.startsWith('Support-') ? 'text-slate-900' : 'text-[#64748b]'}`}>Support Center</span>
                            </div>
                            <ChevronDown size={14} className={`text-slate-400 transition-transform duration-300 ${expandedNav === 'Support' ? 'rotate-180' : ''}`} />
                        </button>
                        <AnimatePresence>
                            {expandedNav === 'Support' && (
                                <motion.div 
                                    ...
                                    {SUPPORT_NAV.map(item => {
                                        ...
                                        return (
                                            <button key={item.id}
                                                onClick={() => { setActiveSection(item.id); setShowMobileSidebar(false); }}
                                                className={`w-full flex items-center gap-4 px-4 py-2.5 transition-all duration-200 text-[13px] font-bold ${isActive ? 'text-[#6366f1] bg-[#6366f1]/5' : 'text-[#64748b] hover:text-[#18181b] hover:bg-slate-100/50'}`}>
                                                <item.icon size={15} className={isActive ? 'text-[#6366f1]' : 'text-[#94a3b8]'} />
                                                {item.label}
                                            </button>
                                        );
                                    })}
                    ...
                    {/* System Section */}
                    <div className="space-y-1.5">
                        <p className="text-[10px] font-black text-[#94a3b8] uppercase tracking-[0.2em] mb-3 ml-4">System</p>
                        {SYSTEM_NAV.map(item => {
                            const isActive = activeSection === item.id;
                            return (
                                <button key={item.id}
                                    onClick={() => { setActiveSection(item.id); setShowMobileSidebar(false); }}
                                    className={`w-full flex items-center justify-between px-5 py-3.5 rounded-full transition-all duration-300 ${isActive ? 'bg-white border shadow-lg border-slate-100/50 scale-[1.02]' : 'hover:bg-slate-50 opacity-70 hover:opacity-100'}`}>
                                    <div className="flex items-center gap-4">
                                         <item.icon size={18} className={isActive ? 'text-blue-600' : 'text-[#94a3b8]'} />
                                         <span className={`text-[14px] font-black ${isActive ? 'text-slate-900' : 'text-[#64748b]'}`}>{item.label}</span>
                                    </div>
                                    {item.badge && (
                                        <span className="bg-blue-50 text-blue-600 px-2.5 py-1 rounded-lg text-[9px] font-black border border-blue-100">{item.badge}</span>
                                    )}
                ...
                <div className="px-6 py-8 border-t border-[#f1f5f9]">
                    <div className="flex items-center justify-between px-3">
                        <div className="flex items-center gap-3 cursor-pointer group" onClick={() => setActiveSection('Admin-Profile')}>
                             <div className={`w-10 h-10 rounded-full flex items-center justify-center transition-all ${activeSection === 'Admin-Profile' ? 'bg-blue-600 shadow-lg scale-110' : 'bg-slate-100 border border-slate-200 group-hover:bg-blue-50 group-hover:border-blue-200'}`}>
                                 <UserIcon size={20} className={activeSection === 'Admin-Profile' ? 'text-white' : 'text-slate-400 group-hover:text-blue-600'} />
                             </div>
                             <div className="flex flex-col">
                                 <span className={`text-sm font-black ${activeSection === 'Admin-Profile' ? 'text-blue-600' : 'text-slate-700 group-hover:text-blue-700'}`}>{currentUser?.name || 'Om Dubey'}</span>
                                 <span className="text-[9px] font-black text-slate-400 uppercase tracking-widest">Active Admin</span>
                             </div>
                        </div>
                        <button onClick={() => navigate('/dashboard')} className="p-2.5 rounded-xl bg-slate-50 text-slate-400 hover:bg-rose-50 hover:text-rose-600 transition-all border border-slate-100 hover:border-rose-100 shadow-sm">
                    ...
                    <AnimatePresence>
                        {status && (
                            <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}
                                className={`mb-5 p-4 rounded-2xl flex items-center gap-3 ${status.type === 'success' ? 'bg-blue-50 text-blue-700 border border-blue-200' : 'bg-amber-50 text-amber-700 border border-amber-200'}`}>
                            ...
                            <>
                                ...
                                {activeSection === 'AllMembers' && (
                                    <AllMembersTable 
                                        ...
                                        handleSyncMembersToCloud={handleSyncMembersToCloud}
                                        isSyncing={isSyncing}
                                    />
                                )}
                                ...
                                {/* Payout Management */}
                                {activeSection === 'Payout-Account' && (
                                    <div className="flex flex-col items-center justify-center min-h-[50vh] bg-white rounded-[2.5rem] border border-slate-100 shadow-sm p-12">
                                        ...
                                        <h2 className="text-2xl font-black text-slate-800 uppercase italic tracking-tight">Payout Account Approval</h2>
                                        <p className="text-slate-400 font-bold text-xs uppercase tracking-widest mt-2">Manage bank account verification for payout services</p>
                                        <div className="mt-8 px-6 py-3 bg-amber-50 text-amber-600 rounded-2xl text-[10px] font-black uppercase tracking-widest border border-amber-200 shadow-sm animate-pulse">Interface Under Development</div>
                                    </div>
                                )}
                                {activeSection === 'Payout-Pending' && (
                                    <div className="flex flex-col items-center justify-center min-h-[50vh] bg-white rounded-[2.5rem] border border-slate-100 shadow-sm p-12">
                                        <div className="w-20 h-20 bg-amber-50 rounded-3xl flex items-center justify-center text-amber-600 mb-6 border border-amber-100">
                                            <ClockIcon size={40} />
                                        </div>
                                        <h2 className="text-2xl font-black text-slate-800 uppercase italic tracking-tight">Pending Account Approval</h2>
                                        <p className="text-slate-400 font-bold text-xs uppercase tracking-widest mt-2">Review and authorize pending payout account requests</p>
                                        <div className="mt-8 px-6 py-3 bg-indigo-50 text-indigo-600 rounded-2xl text-[10px] font-black uppercase tracking-widest border border-indigo-200 shadow-sm animate-pulse">Interface Under Development</div>
                                    </div>
                                )}

                                {/* Other Services Management */}
                                {activeSection === 'List-Services' && <ServicesManager type="list" />}
                                {activeSection === 'Add-Services' && <ServicesManager type="add" />}

                                {/* Settings Management */}
                                {activeSection === 'Settings-Logs' && <SettingsManager type="logs" />}
                                {activeSection === 'Settings-Main' && <SettingsManager type="main" />}
                                {activeSection === 'Settings-Package' && <SettingsManager type="package" />}
                                {activeSection === 'Settings-Commission' && <SettingsManager type="commission" />}
                                {activeSection.startsWith('Txn-') && <TransactionLogsManager type={activeSection.split('-')[1]} />}
                                {activeSection === 'Admin-Profile' && <AdminProfile currentUser={currentUser} refreshData={refreshData} />}

                                {/* Support Management */}
                                {activeSection === 'Support-Pending' && (
                                    <div className="flex flex-col items-center justify-center min-h-[50vh] bg-white rounded-[2.5rem] border border-slate-100 shadow-sm p-12 text-center">
                                        <div className="w-20 h-20 bg-orange-50 rounded-3xl flex items-center justify-center text-orange-600 mb-6 border border-orange-100">
                                            <ClockIcon size={40} />
                                        </div>
                                        <h2 className="text-2xl font-black text-slate-800 uppercase italic tracking-tight">Pending Support Tickets</h2>
                                        <p className="text-slate-400 font-bold text-xs uppercase tracking-widest mt-2">Urgent assistance requests from retailers and distributors</p>
                                        <div className="mt-8 px-6 py-3 bg-orange-50 text-orange-600 rounded-2xl text-[10px] font-black uppercase tracking-widest border border-orange-200 shadow-sm animate-pulse">Ticket Queue Under Development</div>
                                    </div>
                                )}
                                {activeSection === 'Support-Resolved' && (
                                    <div className="flex flex-col items-center justify-center min-h-[50vh] bg-white rounded-[2.5rem] border border-slate-100 shadow-sm p-12 text-center">
                                        <div className="w-20 h-20 bg-blue-50 rounded-3xl flex items-center justify-center text-blue-600 mb-6 border border-blue-100">
                                            <CheckCircle2 size={40} />
                                        </div>
                                        <h2 className="text-2xl font-black text-slate-800 uppercase italic tracking-tight">Resolved Tickets History</h2>
                                        <p className="text-slate-400 font-bold text-xs uppercase tracking-widest mt-2">Closed and approved support requests log</p>
                                        <div className="mt-8 px-6 py-3 bg-blue-50 text-blue-600 rounded-2xl text-[10px] font-black uppercase tracking-widest border border-blue-200 shadow-sm animate-pulse">Support Log Under Development</div>
                                    </div>
                                )}
                            </>
                        )}
                                            ...
                                            <tbody>
                                                {superadmins.map((sa, i) => (
                                                    <tr key={i} className="border-b border-slate-50 hover:bg-indigo-50/30 transition-colors">
                                                        ...
                                                        <td className="px-5 py-3.5 text-xs font-black text-blue-600">₹{sa.wallet?.balance || '0.00'}</td>
                                                        <td className="px-5 py-3.5"><span className={`px-2 py-1 rounded-full text-[9px] font-black uppercase ${sa.status === 'Approved' ? 'bg-blue-50 text-blue-600 border border-blue-200' : 'bg-amber-50 text-amber-600 border border-amber-200'}`}>{sa.status || 'Pending'}</span></td>
                                    ...
                                    {isAeps ? (
                                        <>
                                            <button onClick={() => onApprove(item)}
                                                className="h-11 px-6 bg-blue-500 text-white rounded-[1rem] hover:bg-blue-600 transition-all text-[11px] font-black uppercase tracking-widest shadow-lg shadow-emerald-500/20 active:scale-95 flex items-center gap-2">
                                        ...
                                        <div className="h-11 px-6 bg-slate-900 text-white rounded-[1rem] font-black text-[11px] uppercase tracking-[0.2em] flex items-center gap-3 shadow-xl">
                                            <RefreshCcw size={14} className="animate-spin text-blue-400" /> System_Sync
                                        </div>
                                    ) : (
                                        <>
                                            <button onClick={() => onApprove(item)}
...
const AllMembersTable = ({ 
    ...
    const counts = {
        ...
        LIVE: allMembers.filter(m => m.isRemote).length,
        ...
        <div className="py-4 space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500" style={{ fontFamily: "'Outfit', sans-serif" }}>
            ...
            <div className="bg-white rounded-3xl p-6 border border-slate-100 shadow-[0_4px_25px_rgb(0,0,0,0.02)] flex flex-col md:flex-row items-center justify-between gap-8">
                <div className="flex flex-wrap items-center gap-6 lg:gap-10">
                    {[
                        { label: 'Network Total', val: counts.ALL, color: 'text-indigo-600', bg: 'bg-indigo-50' },
                        { label: 'Proper DB', val: counts.LIVE, color: counts.LIVE === counts.ALL ? 'text-blue-600' : 'text-blue-600', bg: counts.LIVE === counts.ALL ? 'bg-blue-50' : 'bg-blue-50' },
                        { label: 'Retailers', val: counts.Retailer, color: 'text-blue-600', bg: 'bg-blue-50' },
                ...
                <div className="flex flex-col items-end pr-4 border-l border-slate-50 pl-8 ml-auto">
                ...
                <div className="flex items-center gap-3">
                    <button 
                        onClick={handleSyncMembersToCloud}
                        disabled={isSyncing}
                        className={`flex items-center gap-2 px-6 py-4 rounded-2xl text-[10px] font-black uppercase tracking-widest transition-all ${isSyncing ? 'bg-slate-100 text-slate-400 cursor-not-allowed' : 'bg-blue-50 text-blue-600 border border-blue-100 hover:bg-emerald-100 hover:scale-105 active:scale-95 shadow-sm'}`}
                    >
                    ...
                    filtered.map((m) => (
                        ...
                        >
                            ...
                            <div className="flex items-center gap-4 min-w-[240px] shrink-0">
                                ...
                                <div className="truncate">
                                    ...
                                    <div className="flex items-center gap-2 mt-1">
                                        ...
                                        {m.isRemote ? (
                                            <span className="flex items-center gap-1 text-[8px] font-black uppercase tracking-tighter text-blue-500 bg-blue-50 px-2 py-0.5 rounded-full ring-1 ring-blue-200">
                                                <Database size={10} /> Live
                                            </span>
                                        ) : (
                                            <span className="flex items-center gap-1 text-[8px] font-black uppercase tracking-tighter text-amber-500 bg-amber-50 px-2 py-0.5 rounded-full ring-1 ring-amber-200">
                                                <CloudUpload size={10} /> Local Cache (Sync Needed)
                                            </span>
                                        )}
                                    </div>
                                </div>
                            ...
                            <div className="flex items-center justify-end gap-2 w-[120px] shrink-0 ml-auto xl:ml-0">
                                {!m.isRemote && (
                                    <button onClick={() => handleSyncMembersToCloud(m)} disabled={isSyncing}
                                        className="w-10 h-10 flex items-center justify-center bg-amber-50 hover:bg-blue-500 text-amber-600 hover:text-white rounded-xl transition-all" title="Push to DB">
...
// ✨ High-Performance Transaction Monitoring Engine (Glassy UI) ✨
const TransactionLogsManager = ({ type }) => {
    ...
    useEffect(() => {
        const fetchLogs = async () => {
            ...
            try {
                ...
                const keywords = mapping[type] || [];
                const filtered = (allTxns || []).filter(t => 
                    keywords.some(k => t.service?.toUpperCase().includes(k))
                );
                setLogs(filtered);
            } catch (e) {
                console.error("TXN_FETCH_ERROR:", e);
            } finally {
                setLoading(false);
            }
        };
    ...
    return (
        <div className="space-y-8 animate-in fade-in slide-in-from-bottom-8 duration-700">
            ...
            <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-6 p-4 bg-white/40 backdrop-blur-xl rounded-[3rem] border border-white/60 shadow-xl overflow-hidden">
                <div className="px-6 py-2">
                    <h2 className="text-3xl font-black text-slate-900 tracking-tighter italic uppercase flex items-center gap-4">
                        <div className="w-12 h-12 bg-blue-600 rounded-2xl flex items-center justify-center text-white shadow-lg shadow-blue-500/20 transform -rotate-6">
                            <HistoryIcon size={24} />
                        </div>
                        {type} Logs
                    </h2>
                    <p className="text-[10px] font-black text-slate-400 uppercase tracking-[0.3em] mt-2 ml-1">Secure Audit Channel — {filteredLogs.length} Records Detected</p>
                </div>
                ...
                <div className="relative w-full lg:w-[400px]">
                    ...
                    <input 
                        ...
                        placeholder="Search Node, Phone or Operator..."
                        className="w-full bg-white/80 border border-slate-100 rounded-full pl-14 pr-6 py-5 text-sm font-bold shadow-2xl focus:shadow-blue-100 transition-all outline-none"
                    />
                </div>
            </div>

            {/* Main Table Container (Extreme Glassy Surface) */}
            <div className="bg-white/80 backdrop-blur-md rounded-[3.5rem] border border-slate-100/50 shadow-[0_25px_100px_-20px_rgba(0,0,0,0.05)] overflow-hidden">
                <div className="overflow-x-auto">
                    <table className="w-full text-left">
                        <thead>
                            <tr className="bg-slate-50/30 border-b border-slate-100/30">
                                <th className="px-10 py-7 text-[11px] font-black text-slate-400 uppercase tracking-[0.2em]">Temporal Stamp</th>
                                <th className="px-10 py-7 text-[11px] font-black text-slate-400 uppercase tracking-[0.2em]">Merchant ID</th>
                                <th className="px-10 py-7 text-[11px] font-black text-slate-400 uppercase tracking-[0.2em]">Service Details</th>
                                <th className="px-10 py-7 text-[11px] font-black text-slate-400 uppercase tracking-[0.2em] text-right">Transaction Volume</th>
                                <th className="px-10 py-7 text-[11px] font-black text-slate-400 uppercase tracking-[0.2em] text-center">Status Bridge</th>
                            </tr>
                        </thead>
                                ...
                                <tr>
                                    <td colSpan={5} className="py-32 text-center">
                                        <div className="flex flex-col items-center gap-4">
                                            ...
                                            <p className="text-xs font-black text-slate-400 uppercase tracking-[0.5em]">Syncing Encrypted Logs...</p>
                                        </div>
                                    </td>
                                </tr>
                            ) : filteredLogs.length > 0 ? (
                                filteredLogs.map((log, i) => (
                                    <tr key={i} className="hover:bg-blue-50/30 transition-all duration-500 group cursor-default">
                                        <td className="px-10 py-7">
                                            <div className="flex items-center gap-4">
                                                <div className="w-1.5 h-1.5 rounded-full bg-blue-500 shadow-[0_0_10px_rgba(59,130,246,0.5)]" />
                                                <div>
                                                    <p className="text-[13px] font-black text-slate-800 tracking-tight">{new Date(log.created_at || log.timestamp).toLocaleDateString()}</p>
                                                    <p className="text-[10px] font-bold text-slate-400 tracking-widest">{new Date(log.created_at || log.timestamp).toLocaleTimeString()}</p>
                                                </div>
                                            </div>
                                        </td>
                                        <td className="px-10 py-7 font-mono text-[13px] font-extrabold text-blue-600/80 tracking-tighter">{log.userId}</td>
                                        <td className="px-10 py-7">
                                            <p className="text-[12px] font-black text-slate-800 uppercase italic tracking-tight mb-1">{log.service}</p>
                                            <p className="text-[10px] font-bold text-slate-400/80 uppercase tracking-[0.1em]">{log.operator || log.number || log.userId || 'Standard'}</p>
                                        </td>
                                        <td className="px-10 py-7 text-right">
                                            <div className="inline-flex flex-col items-end">
                                                <span className="text-lg font-black text-slate-900 tracking-tighter">₹{parseFloat(log.amount || 0).toLocaleString('en-IN')}</span>
                                            </div>
                                        </td>
                                        <td className="px-10 py-7 text-center">
                                            <span className={`px-6 py-2 rounded-full text-[10px] font-black uppercase tracking-[0.2em] shadow-lg transition-all ${
                                                log.status?.toLowerCase() === 'success' ? 'bg-white text-blue-600 shadow-blue-100 ring-1 ring-blue-50' :
                                                log.status?.toLowerCase() === 'pending' ? 'bg-white text-amber-600 shadow-amber-100 ring-1 ring-amber-50' :
                                                'bg-white text-rose-600 shadow-rose-100 ring-1 ring-rose-50'
                                            }`}>
                                                {log.status || 'SYNCING'}
                                            </span>
                                        </td>
                                ...
                                <tr>
                                    <td colSpan={5} className="py-40 text-center">
                                        <div className="flex flex-col items-center opacity-30">
                                            <HistoryIcon size={40} className="text-slate-400 mb-6" />
                                            <p className="text-sm font-black text-slate-500 uppercase tracking-[0.5em]">No Temporal Data Stream</p>
                                        </div>
                                    </td>
                                </tr>
                            )}
...
// 🏰 Admin Profile - Identity & Security Nexus
const AdminProfile = ({ currentUser, refreshData }) => {
    ...
    const handleUpdate = async () => {
        ...
        setIsUpdating(true);
        // Simulate update or call dataService
        setTimeout(() => {
            alert("Security Profile Synchronized Successfully!");
            setIsUpdating(false);
        }, 1500);
    };
    ...
    return (
        <div className="max-w-4xl mx-auto py-10 animate-in fade-in zoom-in-95 duration-500">
            <div className="bg-white/40 backdrop-blur-2xl rounded-[3rem] border border-white shadow-2xl p-10 relative overflow-hidden">
                {/* Decorative Elements */}
                <div className="absolute top-0 right-0 w-64 h-64 bg-blue-100/30 rounded-full -mr-32 -mt-32 blur-3xl animate-pulse" />
                <div className="absolute bottom-0 left-0 w-48 h-48 bg-indigo-100/20 rounded-full -ml-24 -mb-24 blur-3xl" />

                <div className="relative z-10">
                    <div className="flex items-center gap-8 mb-12">
                        <div className="w-24 h-24 bg-gradient-to-br from-blue-600 to-indigo-700 rounded-[2rem] flex items-center justify-center text-white text-3xl font-black shadow-2xl shadow-blue-500/40 transform -rotate-12 hover:rotate-0 transition-transform duration-500">
                            {name.charAt(0)}
                        </div>
                        <div>
                            <h2 className="text-3xl font-black text-slate-900 uppercase tracking-tighter italic">Administrative Identity</h2>
                            <p className="text-[10px] font-black text-slate-400 uppercase tracking-[0.4em] mt-1">Full Authorization Profile — Level 0 Access</p>
                        </div>
                    </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                        ...
                        <div className="space-y-2">
                            ...
                            <div className="relative group">
                                <UserIcon size={18} className="absolute left-6 top-1/2 -translate-y-1/2 text-slate-400 group-hover:text-blue-500 transition-colors" />
                                <input value={name} onChange={e => setName(e.target.value)}
                                    className="w-full bg-white/60 border border-slate-100/50 rounded-full pl-14 pr-6 py-4 text-sm font-bold shadow-sm focus:shadow-blue-100 transition-all outline-none" />
                            </div>
                        </div>
                        ...
                        <div className="space-y-2">
                            ...
                            <div className="relative group">
                                <Mail size={18} className="absolute left-6 top-1/2 -translate-y-1/2 text-slate-400 group-hover:text-blue-500 transition-colors" />
                        ...
                        <div className="md:col-span-2 mt-4 space-y-6">
                            ...
                            <h3 className="text-xs font-black text-slate-800 uppercase tracking-[0.2em] mb-4">Security Re-Authentication</h3>
                            
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                                <div className="space-y-2">
                                    <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-4">New Access Key</label>
                                    <div className="relative group">
                                        <Lock size={18} className="absolute left-6 top-1/2 -translate-y-1/2 text-slate-400 group-hover:text-blue-500 transition-colors" />
                                        <input type="password" value={newPass} onChange={e => setNewPass(e.target.value)} placeholder="••••••••"
                                            className="w-full bg-white/60 border border-slate-100/50 rounded-full pl-14 pr-6 py-4 text-sm font-bold shadow-sm focus:shadow-blue-100 transition-all outline-none" />
                                    </div>
                                </div>
                                <div className="space-y-2">
                                    <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-4">Verify Key</label>
                                    <div className="relative group">
                                        <ShieldCheck size={18} className="absolute left-6 top-1/2 -translate-y-1/2 text-slate-400 group-hover:text-blue-500 transition-colors" />
                    ...
                    <div className="md:col-span-2 mt-8">
                        <div className="h-px bg-slate-100/50 w-full mb-8" />
                        <h3 className="text-[11px] font-black text-slate-400 uppercase tracking-[0.3em] mb-6 ml-4">System Identity & Telemetry</h3>
                        
                        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                            {[
                                { label: 'Auth Status', value: 'High Privilege', icon: ShieldCheck, color: 'text-blue-600', bg: 'bg-blue-50' },
                                { label: 'Session Length', value: '2h 14m', icon: ClockIcon, color: 'text-amber-600', bg: 'bg-amber-50' },
                                { label: 'Node Access', value: 'Global Entry', icon: MapPin, color: 'text-blue-600', bg: 'bg-blue-50' },
                                { label: 'Net ID', value: '192.168.1.1', icon: ActivityIcon, color: 'text-indigo-600', bg: 'bg-indigo-50' }
                            ].map((stat, i) => (
                                <div key={i} className="p-5 bg-white/60 border border-white rounded-[2rem] shadow-sm flex flex-col gap-3 group hover:scale-[1.03] transition-all">
                    ...
                    <div className="mt-12 flex justify-between items-center bg-blue-50/50 p-6 rounded-[2.5rem] border border-blue-100/50">
                        <div className="flex items-center gap-4">
                            <div className="w-12 h-12 bg-white rounded-2xl flex items-center justify-center text-blue-600 shadow-sm">
                                <ActivityIcon size={24} />
                            </div>
                            <div>
                                <p className="text-[10px] font-black text-blue-900 uppercase tracking-widest">Global Admin Override</p>
                                <p className="text-[9px] font-bold text-blue-600/70">Master terminal access active for this session.</p>
                            </div>
                        </div>
                        <button 
                            onClick={handleUpdate}
                            disabled={isUpdating}
                            className="bg-blue-600 text-white px-10 py-4 rounded-full text-[10px] font-black uppercase tracking-[0.2em] shadow-xl shadow-blue-500/30 hover:shadow-2xl hover:scale-105 active:scale-95 transition-all flex items-center gap-3 disabled:opacity-50"
                        >
